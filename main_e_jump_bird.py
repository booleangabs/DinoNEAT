...
def eval_genomes(genomes, config):
    ...
    while run:
        ...
        is_bird = int(isinstance(obstacle, Bird))
        for i, dinosaur in enumerate(dinosaurs):
            x_diff = (obstacle.rect.x - (dinosaur.rect.x + dinosaur.rect.width)) / SCREEN_WIDTH
            if x_diff < 0:
                x_diff = 1
            output = nets[i].activate((x_diff,
...
def run(config_path, iteration):
    ...
    pickle.dump(best, open(f"nets/simple_jump_bird/{iteration}.pkl", "wb"))
    
    visualize.plot_stats(stats, ylog=False, view=False, filename=f"stats/simple_jump_bird/{iteration}.svg")
    node_names = {-1: 'x_diff', -2: 'object_y', -3: 'object_height', -4: 'is_bird', 0: 'jump'}
    visualize.draw_net(config, best, node_names=node_names, filename=f"graphs/simple_jump_bird/{iteration}")
    

if __name__ == '__main__':
    local_dir = os.path.dirname(__file__)
    config_path = os.path.join(local_dir, 'config_bird.txt')
    for it in range(int(sys.argv[1])):
        run(config_path, it)
    pickle.dump(best_fitnesses, open(f"stats/simple_jump_bird/high_scores.pkl", "wb"))
